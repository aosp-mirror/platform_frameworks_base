...
import com.android.databinding2.BindingTarget
...
import com.android.databinding.util.getCodeName
...
import com.android.databinding2.expr.GroupExpr
import com.android.databinding2.expr.MathExpr
...
fun Expr.isObservable() = ClassAnalyzer.getInstance().isObservable(this.getResolvedType())
...
val BindingTarget.readableUniqueName by Delegates.lazy {(target: BindingTarget) ->
    val stripped = target.getId().extractAndroidId().stripNonJava()
...
val BindingTarget.androidId by Delegates.lazy { (target : BindingTarget) ->
    "R.id.${target.getId().extractAndroidId()}"
}
...
fun Expr.toCode(full : Boolean = false) : KCode {
    ...
    return when (it) {
        ...
        is FieldAccessExpr -> kcode("") {
            ...
            if (it.getGetter().type == ClassAnalyzer.Callable.Type.FIELD) {
...
fun Expr.readNow(mJustRead : List<Expr>) : Boolean =
    !getShouldReadFlags().isEmpty() && !getDependencies().any {! (it.getOther().isRead() || mJustRead.contains(it.getOther()))}
...
class LayoutBinderWriter(val layoutBinder : LayoutBinder, val br : BrRenderer) {
    val model = layoutBinder.getModel()
    val mDirtyFlags = FlagSet(BitSet(), model.getFlagBucketCount());
    {
        Arrays.fill(mDirtyFlags.buckets, -1)
        mDirtyFlags.setDynamic(true)
        model.localizeFlag(mDirtyFlags, "mDirtyFlags")
    }

    val dynamics = model.getExprMap().values().filter { it.isDynamic() }
    val className = layoutBinder.getClassName()

    val identifiers = dynamics.filter { it is IdentifierExpr }

    val interfaceName = "${layoutBinder.getBaseClassName()}"

    val variables= model.getExprMap().values().filterIsInstance(javaClass<IdentifierExpr>()).filter { it.isVariable() }

    val classAnalyzer = ClassAnalyzer.getInstance();
    ...
    fun declareConstructor() = kcode("") {
        tab("public ${className}(View root) {") {
    ...
    fun declareInvalidateAll() = kcode("") {
        tab("protected void invalidateAll() {") {
            val bs = BitSet()
    ...
    fun declareSetVariable() = kcode("") {
        tab("public boolean setVariable(int variableId, Object variable) {") {
            tab("switch(variableId) {") {
                variables.forEach {
                    tab ("case ${br.toIntS(it.getName())} :") {
    ...
    fun declareLog() = kcode("") {
        tab("private void log(String msg, long i) {") {
    ...
    fun variableSettersAndGetters() = kcode("") {
        variables.forEach {
            tab("public void ${it.setterName}(${it.getResolvedType().toJavaCode()} ${it.readableUniqueName}) {") {
                if (it.isObservable()) {
            ...
            tab("public ${it.getResolvedType().toJavaCode()} ${it.getterName}() {") {
    ...
    fun declareViews() = kcode("// views") {
        layoutBinder.getBindingTargets().forEach {
            tab("private ${it.getViewClass()} ${it.fieldName};")
        }
    }
    ...
    fun declareVariables() = kcode("// variables") {
        variables.forEach {
            tab("private ${it.getResolvedType().toJavaCode()} ${it.fieldName};")
        }
    }
    ...
    fun rebindDirty() = kcode("") {
        ...
        tab("public void rebindDirty() {") {
            ...
            do {
                ...
                while (!batch.none()) {
                    val readNow = batch.filter { it.readNow(mJustRead) }
                    if (readNow.isEmpty()) {
                    ...
                    readNow.forEach {
                        tab(readWithDependants(it, mJustRead, batch, tmpDirtyFlags))
                    }
    ...
    fun readWithDependants(expr : Expr, mJustRead : MutableList<Expr>, batch : MutableList<Expr>, tmpDirtyFlags : FlagSet) : KCode = kcode("") {
        ...
        tab("if (${tmpDirtyFlags.mapOr(flagSet){ suffix, index ->
            ...
            val chosen = expr.getDependants().filter {
                batch.contains(it.getDependant()) && it.getDependant().readNow(mJustRead)
            }
