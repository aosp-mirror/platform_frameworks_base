...
public class OverheatAlarmDialog extends AlertDialog {
    ...
    public void setIconAttribute(int attrId) {
        ...
        final ImageView iconView = mTitleView.findViewById(android.R.id.icon);
        if (iconView != null) {
    ...
    public void setMessage(CharSequence message) {
        ...
        final TextView helpLink = mContentView.findViewById(R.id.help_link);
        if (helpLink != null) {
            final String contextString = mContext.getString(R.string.high_temp_alarm_help_url);
            helpLink.setMovementMethod(LinkMovementMethod.getInstance());
            helpLink.setText(getClickableSpanText(R.string.high_temp_alarm_help_care_steps,
                    contextString));
        }
    }

    private SpannableString getClickableSpanText(int stringRes, String contextString) {
        SpannableString ss = new SpannableString(mContext.getString(stringRes));
        ClickableSpan cs = new ClickableSpan() {
            ...
            public void onClick(View view) {
                ...
                mContext.startActivityAsUser(helpIntent,
    ...
    protected void notifyUserUnlock() {
        final String hintMessage = mContext.getString(R.string.keyguard_indication_trust_granted,
                mContext.getString(R.string.high_temp_alarm_help_care_steps));
        Toast notifyUserUnlock = Toast.makeText(mContext, hintMessage, Toast.LENGTH_LONG);
        notifyUserUnlock.getWindowParams().type = WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL;
        notifyUserUnlock.getWindowParams().privateFlags |=
                WindowManager.LayoutParams.PRIVATE_FLAG_SHOW_FOR_ALL_USERS;
        notifyUserUnlock.getWindowParams().flags |=
                WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED;
        notifyUserUnlock.show();
    }
