...
public class PowerUI extends SystemUI {
    ...
    private boolean mIsShowingOverheatAlarm;
    ...
    /**
     * Update Temperature depend on config
    ...
    /**
     * Update legacy skin temperature overheat warning notification
     *
     * @param temps The temperature value of DEVICE_TEMPERATURE_SKIN
     */
    @VisibleForTesting
    ...
    /**
     * Update Temperature by Type
     *
     * @param temps TEMPERATURE_OVERHEAT_WARNING Send generic notification to notify user
     *              TEMPERATURE_OVERHEAT_ALARM Popup emergency Dialog for user
     */
    @VisibleForTesting
    protected void updateTemperatureAlarm(float[] temps) {
        if (temps.length != 0) {
            ...
            if (temp >= mThresholdAlarmTemp && !mIsShowingOverheatAlarm) {
                mWarnings.notifyHighTemperatureAlarm(true);
                mIsShowingOverheatAlarm = true;
            } else if (temp <= mThresholdAlarmTempTolerance && mIsShowingOverheatAlarm) {
                mWarnings.notifyHighTemperatureAlarm(false);
                mIsShowingOverheatAlarm = false;
            }
    ...
    private void logAtTemperatureThreshold(int type, float temp, float thresholdTemp) {
        ...
        sb.append("currentTemp=").append(temp)
                ...
                .append(",isOverheatAlarm=").append(mIsShowingOverheatAlarm)
    ...
    public interface WarningsUI {
         ...
         *
         * @param overheat true if device temp over threshold
         */
        void notifyHighTemperatureAlarm(boolean overheat);
