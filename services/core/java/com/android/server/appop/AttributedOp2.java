...
final class AttributedOp2 {
    private final @NonNull AppOpsServiceImpl mAppOpsServiceImpl;
     ...
     */
    // TODO(b/248108338)
    // @GuardedBy("mAppOpsCheckingServiceImpl")
    ...
    AttributedOp2(@NonNull AppOpsServiceImpl AppOpsServiceImpl, @Nullable String tag,
            @NonNull AppOpsServiceImpl.Op parent) {
        mAppOpsServiceImpl = AppOpsServiceImpl;
        this.tag = tag;
        this.parent = parent;
    }
    ...
    public void accessed(int proxyUid, @Nullable String proxyPackageName,
        ...
        mAppOpsServiceImpl.mHistoricalRegistry.incrementOpAccessedCount(parent.op, parent.uid,
     ...
     * @param flags               OpFlags of the call
     */
    @SuppressWarnings("GuardedBy") // Lock is held on mAppOpsCheckingServiceImpl
    public void accessed(long noteTime, long duration, int proxyUid,
        ...
        if (proxyUid != Process.INVALID_UID) {
            proxyInfo = mAppOpsServiceImpl.mOpEventProxyInfoPool.acquire(proxyUid, proxyPackageName,
                    proxyAttributionTag);
        }
        ...
        if (existingEvent != null) {
            existingEvent.reinit(noteTime, duration, proxyInfo,
                    mAppOpsServiceImpl.mOpEventProxyInfoPool);
        } else {
    ...
    public void rejected(@AppOpsManager.UidState int uidState, @AppOpsManager.OpFlags int flags) {
        ...
        mAppOpsServiceImpl.mHistoricalRegistry.incrementOpRejected(parent.op, parent.uid,
    ...
    public void rejected(long noteTime, @AppOpsManager.UidState int uidState,
        ...
        if (existingEvent != null) {
            existingEvent.reinit(noteTime, -1, null, mAppOpsServiceImpl.mOpEventProxyInfoPool);
        } else {
    ...
    private void startedOrPaused(@NonNull IBinder clientId, int proxyUid,
        ...
        if (triggerCallbackIfNeeded && !parent.isRunning() && isStarted) {
            mAppOpsServiceImpl.scheduleOpActiveChangedIfNeededLocked(parent.op, parent.uid,
        ...
        if (event == null) {
            event = mAppOpsServiceImpl.mInProgressStartOpEventPool.acquire(startTime,
    ...
    private void finishOrPause(@NonNull IBinder clientId, boolean triggerCallbackIfNeeded,
        ...
        if (event.mNumUnfinishedStarts == 0 || isPausing) {
            ...
            mAppOpsServiceImpl.mHistoricalRegistry.increaseOpAccessDuration(parent.op, parent.uid,
                    parent.packageName, tag, event.getUidState(),
                    event.getFlags(), finishedEvent.getNoteTime(), finishedEvent.getDuration(),
                    event.getAttributionFlags(), event.getAttributionChainId());

            if (!isPausing) {
                mAppOpsServiceImpl.mInProgressStartOpEventPool.release(event);
                if (mInProgressEvents.isEmpty()) {
                    ...
                    if (triggerCallbackIfNeeded && !parent.isRunning()) {
                        mAppOpsServiceImpl.scheduleOpActiveChangedIfNeededLocked(parent.op,
    ...
    public void resume() {
        ...
        for (int i = 0; i < mPausedInProgressEvents.size(); i++) {
            ...
            mAppOpsServiceImpl.scheduleOpStartedIfNeededLocked(parent.op, parent.uid,
    ...
    void onClientDeath(@NonNull IBinder clientId) {
        synchronized (mAppOpsServiceImpl) {
     ...
     */
    @SuppressWarnings("GuardedBy") // Lock is held on mAppOpsCheckingServiceImpl
    public void add(@NonNull AttributedOp2 opToAdd) {
        if (opToAdd.isRunning() || opToAdd.isPaused()) {
            ...
            Slog.w(AppOpsServiceImpl.TAG, "Ignoring " + ignoredEvents.size() + " app-ops, running: "
                    + opToAdd.isRunning());
